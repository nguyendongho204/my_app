...
class _QuanLiDonVeState extends State<QuanLiDonVe> {
  ...
  Future<void> _tai() async {
    ...
    try {
      // Lấy danh sách vé theo ngày lọc để phục vụ quản lý đơn trong ca hiện tại.
  ...
  List<Ve> get _hienThi {
    // Áp dụng lọc theo trạng thái trước, sau đó mới lọc theo từ khóa tìm kiếm.
  ...
  void _chiTiet(Ve ve) {
    NguoiDung? kh;
    bool dangTaiKH = ve.nguoiDungId.isNotEmpty;
    ...
    showCupertinoModalPopup(
      ...
      builder: (_) => StatefulBuilder(
        builder: (ctx, setM) {
          if (dangTaiKH) {
            // Lazy-load thông tin khách hàng khi mở modal để tránh query thừa.
            dangTaiKH = false;
            CoSoDuLieu().layNguoiDungTheoId(ve.nguoiDungId).then((nd) {
              if (ctx.mounted) setM(() => kh = nd);
            });
          }
          return Container(
