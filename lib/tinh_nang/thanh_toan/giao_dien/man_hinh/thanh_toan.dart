...
class _ThanhToanState extends State<ThanhToan> {
  // Input người đi và mã khuyến mãi.
  ...
  // Thành tiền cuối cùng sau khi cộng phí dịch vụ và trừ giảm giá.
  ...
  Future<void> _taiCauHinh() async {
    try {
      // Đọc cấu hình hệ thống (ví dụ: phí dịch vụ) từ Firestore.
  ...
  Future<void> _apDungKM() async {
    ...
    // Gọi backend để kiểm tra mã, tính tiền giảm và phản hồi lỗi nếu có.
  ...
  void _xacNhan() async {
    // Validate thông tin hành khách trước khi thanh toán.
    ...
    if (TrangThaiUngDung().daDangNhap) {
      try {
        final db = CoSoDuLieu();
        // Tạo bản ghi vé và lưu vào DB khi người dùng đã đăng nhập.
